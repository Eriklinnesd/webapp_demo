// Fikk feilmelding på import så jeg kopierte objektet fra CodeSandbox
const users = [
...
const handleSearch = () => {
  ...
  if (searchName) {
    ...
    const searchResult = users.find(e => e.name == searchName);
    ...
    if (searchResult) {
      ...
      userUl.innerHTML = `<p> Det er dessverre ingen brukere med navn ${searchName} </p>`
    }
  } else {
...
const handleFilter = () => {
  ...
  const filterValue = document.getElementById('age').value;
  ...
  if (filterValue && Number(filterValue)) {
    ...
    if (filterResult && filterResult.length > 0) {
      ...
      // TODO: Oppdatere grensesnittet med userUl.innerHTML og en passende tekst når vi ikke finner noe
      userUl.innerHTML = `<p> Det er dessverre ingen brukere over ${filterValue} år gammel</p>`
    }
  } else {
